...
The resulting overall process is depicted in Figure \ref{fig_overallprocess}. After the construction of the search tree as explained in section \ref{sec_searchtreeconstruction}, cooccurring tags and images are added to all nodes and nodes merged if necessary (section \ref{sec_picturestonodes}). \\
The subclustering explained below will only take place for nodes/clusters with a certain minimum size and results in the structure of three nested Arrays of the WordnetNode class' attribute \emph{subclusters} shown in figure \ref{fig_nodestructure}.
